...
const currentUserId = useSelector((state) => state.auth.userId);
  ...
  const carImage = car.imageUrl || car.image || "https://via.placeholder.com/280x155?text=No+Image";
            ...
            <div className="row g-0 align-items-center" style={{ minHeight: "320px" }}>
              <div className="col-lg-5 col-md-5 text-center py-4">
                <img
                  ...
                  style={{ maxWidth: 280, height: 155, objectFit: "cover" }}
                  loading="lazy"
              ...
              <div className="col-lg-7 col-md-7 px-4 py-4" style={{ minHeight: "190px", display: "flex", flexDirection: "column", justifyContent: "center" }}>
                <div className="d-flex align-items-baseline mb-3 gap-4 flex-wrap">
                  ...
                  <span className="fw-bold fs-5" style={{ color: "#16a085" }}>₹{car.pricePerDay}</span>
                ...
                <form className="row row-cols-lg-auto g-3 align-items-end" onSubmit={handleBookNow} autoComplete="off">
                  <div className="col-12">
                    <label htmlFor="startDate" className="form-label text-primary fw-semibold">Start</label>
                    <input type="date" className="form-control" id="startDate" value={startDate} min={todayString()} onChange={e => setStartDate(e.target.value)} required />
                  </div>
                  <div className="col-12">
                    <label htmlFor="endDate" className="form-label text-primary fw-semibold">End</label>
                    <input type="date" className="form-control" id="endDate" value={endDate} min={startDate ? startDate : todayString()} onChange={e => setEndDate(e.target.value)} required />
                  </div>
                  <div className="col-12 mt-3 mt-lg-0">
                    <button type="submit" className="btn fw-semibold px-4 py-2 text-white" style={{ background: "#112266", borderRadius: "0.5rem", fontWeight: "600", fontSize: "1rem" }}
                    ...
                    <div className="col-12">
                      ...
                      {overlappingRanges.length > 0 && (
                        <div className="mt-2">
                          ...
                          <ul className="mb-0">
                            {overlappingRanges.map((range, idx) => (<li key={idx}>{range.start} to {range.end}</li>))}
                          </ul>
                        </div>
                      )}
                    </div>
                ...
                {unavailableRanges.length > 0 && (
                  <div className="alert alert-warning mt-4" style={{ fontSize: "1rem" }}>
                    ...
                    <ul className="mb-0">
                      {unavailableRanges.map((range, idx) => (<li key={idx}>{range.start} to {range.end}</li>))}
                    </ul>
                  </div>
                )}
              </div>
